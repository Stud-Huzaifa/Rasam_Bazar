  ...
  return (
    <VendorShell>
      <section className="rounded-3xl border border-white/10 bg-white/5 p-8 md:p-10">
        ...
        <h1 className="mt-2 text-3xl font-semibold">Customer conversations</h1>
        <p className="mt-4 max-w-2xl text-slate-300">
          Reply to booking customers and track alerts from proposal, payment,
          review, and booking workflows.
        </p>
        {loading ? (
          <p className="mt-6 text-sm text-slate-400">Loading messages...</p>
        ) : null}
        {error ? (
        ...
        <div className="mt-8 grid gap-6 xl:grid-cols-[0.8fr_1.2fr]">
              ...
              <div className="mt-4 space-y-3">
                {threads.map((thread) => (
                  <button
                    ...
                    type="button"
                    className={`block w-full rounded-xl border p-4 text-left ${selectedThreadId === thread.id ? 'border-amber-400/60 bg-amber-400/10' : 'border-white/10 bg-white/5 hover:bg-white/10'}`}
                  >
                ...
                {!loading && threads.length === 0 ? (
                  <p className="rb-empty text-sm">No booking threads yet.</p>
                ) : null}
              </div>
            </div>
            <div className="rounded-2xl border border-white/10 bg-slate-900/70 p-5">
              <h2 className="text-xl font-semibold">Open from booking</h2>
              <div className="mt-4 space-y-3">
          ...
          <div className="space-y-6">
            <div className="rounded-2xl border border-white/10 bg-slate-900/70 p-5">
              ...
              <div className="mt-5 max-h-[440px] space-y-3 overflow-y-auto pr-2">
            ...
            <div className="rounded-2xl border border-white/10 bg-slate-900/70 p-5">
              ...
              <div className="mt-4 space-y-3">
                ...
                {!loading && notifications.length === 0 ? (
                  <p className="rb-empty text-sm">No alerts yet.</p>
                ) : null}
              </div>
            </div>
          </div>
